...
import { addAttributes, addCategory } from "../../api/ApiClient";
...
const AddAttribute = () => {
  ...
  const addAttributesMutataion = useMutation((data) => addAttributes(data), {
    onSuccess: (res) => {
      navigate(-1);
      formik.resetForm();
    },
  });
  const [enabled] = useStrictDroppable(addAttributesMutataion?.isLoading);
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      options.length === 0
        ? ""
        : addAttributesMutataion.mutate(values);
    },
    validate: (res) => {
        ...
        <div className="w-full">
          <form onSubmit={formik.handleSubmit}>
            <div className="w-full bg-blue-lightt rounded-lg p-6 mt-4 flex flex-col gap-5 ">
              ...
              >
                {addAttributesMutataion.isLoading ? (
