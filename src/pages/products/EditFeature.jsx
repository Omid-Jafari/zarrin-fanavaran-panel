...
import { singleFeature, uodateFeature } from "../../api/ApiClient";
...
const EditFeature = () => {
  ...
  const { id } = useParams();
  const getFeatureQuery = useQuery(
    ["getFeatureQuery"],
    async () => singleFeature(id),
    {
      onSuccess: (res) => setFeatureData(res?.data?.data),
    }
  );
  const addFeatureMutataion = useMutation((data) => uodateFeature(data), {
    onSuccess: (res) => {
      navigate(-1);
      formik.resetForm();
    },
  ...
  const formik = useFormik({
    initialValues: {
      ...
      icon_id: featureData?.media?.icon?.id || "",
    },
    enableReinitialize: true,
    validationSchema: () => ADD_FEATURE_VALIDATION_SCHEMA(),
    onSubmit: (values) => {
      addFeatureMutataion.mutate({ id, body: values });
    },
  });
  ...
  useEffect(() => {
    setshowIconMedia(featureData?.media?.icon?.file);
  }, [featureData]);
  return (
