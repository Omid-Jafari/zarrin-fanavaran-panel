...
import { editColor, singleColor } from "../../api/ApiClient";
...
const EditColor = () => {
  ...
  const getColorQuery = useQuery(
    ["getColorQuery"],
    async () => singleColor(id),
    {
      onSuccess: (res) => setColorData(res?.data?.data),
    }
  );
  const editColorMutataion = useMutation(editColor, {
    onSuccess: (res) => {
      navigate(-1);
      formik.resetForm();
    },
  ...
  const formik = useFormik({
    initialValues: {
      name_fa: colorData?.name_fa || "",
      name_en: colorData?.name_en || "",
      icon_id: colorData?.media?.icon?.id || "",
    },
    enableReinitialize: true,
    validationSchema: () => ADD_COLOR_VALIDATION_SCHEMA(),
    onSubmit: (values) => {
      editColorMutataion.mutate({ id, body: values });
    },
  });
  ...
  useEffect(() => {
    setshowIconMedia(colorData?.media?.icon?.file);
  }, [colorData]);
  return (
