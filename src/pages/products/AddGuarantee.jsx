...
import { addGuarantee } from "../../api/ApiClient";
import Loading from "../../components/elements/loading";

const AddGuarantee = () => {
  const navigate = useNavigate();
  const addGuaranteeMutataion = useMutation((data) => addGuarantee(data), {
    onSuccess: (res) => {
      navigate(-1);
      formik.resetForm();
    },
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      addGuaranteeMutataion.mutate(values);
    },
  });
      ...
      <div className="w-full">
        <form onSubmit={formik.handleSubmit}>
          <div className="w-full bg-blue-lightt rounded-lg p-6 mt-4 flex flex-col gap-5 ">
            ...
            >
              {addGuaranteeMutataion.isLoading ? (
