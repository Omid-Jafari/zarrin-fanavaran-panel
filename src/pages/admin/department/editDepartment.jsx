...
import { editDepartment, singleDepartment } from "../../../api/ApiClient";
...
const EditDepartment = () => {
  ...
  const departmentDataQuery = useQuery(
    ["departmentDataQuery"],
    () => singleDepartment(id),
    {
      onSuccess: (res) => {
  ...
  const editDepartmentMutataion = useMutation(
  ...
  const formik = useFormik({
    initialValues: {
      ...
      admin_ids: departmentData?.admins?.map((admin) => admin?.id),
    },
    ...
    onSubmit: (values) => {
      editDepartmentMutataion.mutate(values);
    },
  });
  return (
    <>
      <div className="w-full p-5">
        ...
        {departmentDataQuery?.isLoading ? (
          <Loading className="w-11 h-11 text-primary animate-pulse" />
        ) : (
          <form className="w-full bg-blue-lightt rounded-lg p-6 mt-4 flex flex-col gap-5 ">
            ...
            >
              {editDepartmentMutataion.isLoading ? (
