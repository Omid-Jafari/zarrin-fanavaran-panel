...
import { addRole, dataPermissions } from "../../../api/ApiClient";
...
const AddRoles = () => {
  ...
  const addRoleMutataion = useMutation((data) => addRole(data), {
    onSuccess: (res) => {
      navigate(-1);
      formik.resetForm();
    },
      ...
      <div className="w-full p-5">
        ...
        <form className="w-full bg-blue-lightt rounded-lg p-6 mt-4 flex flex-col gap-5 ">
          ...
          {formik.errors.permissions && formik.touched.permissions && (
            <small className="text-red-700">{formik.errors.permissions}</small>
          )}
          <button
