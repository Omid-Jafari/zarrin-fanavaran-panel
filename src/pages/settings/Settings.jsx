...
import { getSettings } from "../../api/ApiClient";
import MediaSettingItem from "../../components/settings/MediaSettingItem";
import NumberSettingItem from "../../components/settings/NumberSettingItem";
import SelectBoxRemoteSettingItem from "../../components/settings/SelectBoxRemoteSettingItem";
import SelectBoxSettingItem from "../../components/settings/SelectBoxSettingItem";
import TextAreaSettingItem from "../../components/settings/TextAreaSettingItem";
import TextSettingItem from "../../components/settings/TextSettingItem";

function Settings() {
  const [settings, setSettings] = useState({});
  const [selectedSetting, setselectedSetting] = useState("");
  const [settingData, setSettingData] = useState([]);
  const getSettingsQuery = useQuery(["getSettingsQuery"], () => getSettings(), {
    onSettled: (res) => {
      setSettings(res?.data?.data);

      // handleSelectedSetting(Object.entries(res?.data?.data)[0][0], Object.entries(res?.data?.data)[0][1])
    },
    refetchOnWindowFocus: false,
  });

  const handleSelectedSetting = (object, value) => {
    setselectedSetting(object);
    setSettingData(value);
    console.log("sdvsdvsdvsdvsd", value);
  };

  const handleItem = (settingItem) => {
    if (settingItem.type == "text") {
      return <TextSettingItem settingItem={settingItem} />;
    } else if (settingItem.type == "textarea") {
      return <TextAreaSettingItem settingItem={settingItem} />;
    } else if (settingItem.type == "number") {
      return <NumberSettingItem settingItem={settingItem} />;
    } else if (settingItem.type == "select") {
      return <SelectBoxSettingItem settingItem={settingItem} />;
    } else if (settingItem.type == "select_with_remote_data") {
      return <SelectBoxRemoteSettingItem settingItem={settingItem} />;
    } else if (settingItem.type == "media") {
      return <MediaSettingItem settingItem={settingItem} />;
    }
  };
  return (
    <div className="w-full h-full p-5">
      ...
      <div className="flex flex-col w-full ">
        <div className="w-full flex flex-row gap-5 rounded-lg bg-[#DBEEF6] mt-3 p-5">
          {Object.entries(settings).map((sett) => (
            <div
              className={`flex flex-row items-center w-[122px] font-Kalameh font-[400] cursor-pointer ${
                sett[0] == selectedSetting
                  ? "bg-[#478F95] text-[#F9FCFD]"
                  : "bg-white"
              } h-[44px] rounded justify-center`}
              onClick={() => handleSelectedSetting(sett[0], sett[1])}
            >
              {sett[0]}
            </div>
          ))}
        ...
        <div className="w-full mt-8">
          {settingData?.map((settingItem) => (
            <div className="w-full flex flex-col gap-5 rounded-lg bg-[#DBEEF6] mt-4 p-5">
              <span>{settingItem?.name}</span>
              {handleItem(settingItem)}
              {/* <TextSettingItem settingItem={settingItem}/> */}
              {/* <TextAreaSettingItem settingItem={settingItem}/> */}
              {/* <NumberSettingItem settingItem={settingItem}/> */}
              {/* <SelectBoxSettingItem settingItem={settingItem}/> */}
              {/* <SelectBoxRemoteSettingItem settingItem={settingItem}/> */}
            </div>
          ))}
        </div>
      </div>
