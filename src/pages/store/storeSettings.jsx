...
import SecondSettingSection from "./storeSettingSections/secondSettingSection";
...
const StoreSettings = () => {
  ...
  const newCouponRef = useRef();
  const openNewCouponModal = () => {
    newCouponRef.current.openModal();
  };
  const sectionTitles = [
    "محصولات",
    "کد تخفیف",
  ...
  const sectionItems = [
    <SecondSettingSection id={id} newCouponRef={newCouponRef} />,
    <SecondSettingSection id={id} newCouponRef={newCouponRef} />,
  ];
  return (
    <>
      <div className="p-4 flex flex-col gap-4">
        ...
        <div className="w-full flex items-center gap-3 rounded-lg bg-blue-lightt px-3 py-4">
          {sectionTitles?.map((title, index) => (
            <button
              onClick={() => navigate(`/store-setting?section=${index}`)}
