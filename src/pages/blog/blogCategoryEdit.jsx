...
import { editBlogCats, singleBlogCat } from "../../api/ApiClient";
...
function BlogCategoryEdit() {
  ...
  const singleBlogCatQuery = useQuery(
    ["singleBlogCatQuery"],
    () => singleBlogCat(id),
    {
      onSuccess: (res) => {
        setBlogCatData(res.data?.data);
      },
    }
  );
  const editBlogCatsMutataion = useMutation(
    (data) => editBlogCats({ id, data }),
    {
      onSuccess: (res) => {
        navigate(-1);
        formik.resetForm();
  ...
  const formik = useFormik({
    initialValues: {
      ...
      slug: blogCatData?.slug || "",
      meta_title: blogCatData?.meta_title || "",
      meta_keywords: blogCatData?.meta_keywords || [],
      meta_description: blogCatData?.meta_description || "",
      canonical: blogCatData?.canonical || "",
    },
    enableReinitialize: true,
    validationSchema: ADD_BLOG_CATEGORY_VALIDATION_SCHEMA,
    onSubmit: (values, { errors }) => {
      values.slug = slugPersian(formik.values.name);
      editBlogCatsMutataion.mutate(values);
    },
    validate: (res) => {},
  });
  console.log("blogCatData", blogCatData);
  return (
      ...
      <div>
        <form onSubmit={formik.handleSubmit}>
          <div className="w-full bg-blue-lightt rounded-lg pt-14 px-4 pb-4 mt-4 flex flex-col gap-5 ">
            ...
            >
              {editBlogCatsMutataion.isLoading ? (
