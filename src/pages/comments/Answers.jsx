...
import { getAnswers } from "../../api/ApiClient";
...
import AnswersItem from "../../components/comments/answers/AnswersITem";
import PermissionContext from "../../context/permisions/PermissionContext";
...
function Answers() {
  ...
  const ref=useRef()
  const getAnswersQuery = useMutation((data) => getAnswers(data), {
    onSuccess: (res) => {
  ...
  const getNextPage = () => {
    ...
    getAnswersQuery.mutate({
      user_id: user?.user?.id,
      status: JSON.parse(localStorage.getItem("answerStatus")).status,
      page: page,
    });
  };
  ...
  const { isLogin, dispatch } = useContext(PermissionContext);
  dispatch({
  type: "SET_PERMISSIONS",
  permissions: ["scascascasc"],
});
  return (
      ...
      <div className="w-full h-full mt-6 relative ">
        <div className="w-full bg-[#DBEEF6] rounded-lg p-4 flex flex-row justify-between">
          ...
          <div className="flex flex-row  ">
            ...
            <StatusFilter
              getCommentsQuery={getAnswersQuery}
        ...
        <div className="w-full mt-5">
          <InfiniteScroll
            ...
            next={() => getNextPage()}
            hasMore={
              getAnswersQuery?.data?.data?.meta?.last_page <=
              getAnswersQuery?.data?.data?.meta?.current_page
                ? false
                : true
            }
            // hasMore={true}
