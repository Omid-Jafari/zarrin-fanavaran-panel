...
import { attributesFilterData } from "../../api/ApiClient";
import Loading from "../elements/loading";

const AttributesFilter = (props) => {
  const { setFilterAttributeIds, filterAttributeIds, subMenu, openSubmenu } =
    props;
  const [filtersAttributeData, setFiltersAttributeData] = useState();
  const [filterAttributeInput, setFilterAttributeInput] = useState("");
  const attributesFilterDataMutation = useMutation(attributesFilterData, {
    onSuccess: (res) => {
      setFiltersAttributeData(res?.data?.data);
    },
  });
  useEffect(() => {
    attributesFilterDataMutation?.mutate({ filterData: filterAttributeInput });
  }, [filterAttributeInput]);
  console.log("filtersAttributeData", filtersAttributeData);
  return (
    <div className="bg-white rounded-lg p-3 mt-3 flex flex-col">
      <h6 className="text-sm">ویژگی های محصول&nbsp;:</h6>
      <div className="flex items-center bg-[#EFF1F1] rounded-lg h-10 w-full p-2 mt-3">
        <input
          type="text"
          placeholder="جستجوی ویژگی های محصول"
          className="flex-1 focus:outline-none ml-2 bg-transparent placeholder:text-[#C4C7C7] text-sm"
          onChange={(e) => setFilterAttributeInput(e?.target?.value)}
          value={filterAttributeInput}
        />
        <svg
      ...
      {attributesFilterDataMutation?.isLoading ? (
        <div className="w-full flex justify-center">
          <Loading className="w-14 h-14 text-blacklead animate-pulse" />
        </div>
      ) : (
        filtersAttributeData?.map((filter) => (
          <div
            key={filter?.id}
          ...
          >
            <div className="flex items-center w-full">
              <input
                ...
                checked={
                  filterAttributeIds.findIndex((aId) => aId === filter?.id) ===
                  -1
                    ? false
                    : true
                }
                onChange={() =>
                  setFilterAttributeIds((prev) => {
                    const attributeId = prev.findIndex(
                      (aId) => aId === filter?.id
                    );
                    ...
                    if (attributeId === -1) {
                      ...
                      return [
                        ...prev.slice(0, attributeId),
                        ...prev.slice(attributeId + 1),
                      ];
                    }
                  })
                }
