...
function TicketsStatusFilter({ getTicketsData,setComments }) {
  ...
  const handleChangeStatus = (item) => {
    ...
    // setComments([])
    localStorage.setItem(`ticketStatus`,JSON.stringify(item))
      getTicketsData.mutate(
        {
            // user_id: user?.user?.id,
            department_id: localStorage.getItem("ticketDepartment")?localStorage.getItem("ticketDepartment"):1,
            status: item?.status,
            page: 1,
        }
     )
  ...
  useEffect(() => {
    ...
    return () => localStorage.removeItem(`ticketStatus`)
...
export default TicketsStatusFilter;
