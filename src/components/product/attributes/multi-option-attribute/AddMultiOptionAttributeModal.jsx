...
function AddMultiOptionAttributeModal({}, ref) {
  ...
  const handleSelectedAttribute = (item, index) => {
    // delete options[index];
    setoptions((prev) => prev.filter((item, indexx) => indexx != index));
    setselectedttibute((prev) => [...prev, item]);
    console.log("vsdvdsv", selectedttibute);
    formik.setFieldValue("option_ids", [item?.id]);
    formik.setFieldValue("value", item?.name_fa);
  };

  const handleDeleteOption = (item, index) => {
    console.log("svsdvsdvsdv", item);
    console.log("kjkjbkjb", options);
    setoptions((prev) => [...prev, item]);
    console.log("vdsvdsv", options);
    setselectedttibute((prev) =>
  ...
  const handleSearchOption = (s) => {
    ...
    setoptions(
      attrData?.options.filter((op) => op?.name_fa.toLowerCase().match(re))
    );
  };
                        ...
                        <div className=" w-1/2 bg-[#DBEEF6] p-2 mt-4 rounded-[6px]">
                          <div className="w-full z-10 flex flex-col justify-start items-start gap-[18px] overflow-scroll hide-scrollbar h-96 p-2">
                            ...
                            {options?.map((op, index) => (
                              <div
                                onClick={() =>
                                  handleSelectedAttribute(op, index)
                                }
                        ...
                        <div className=" w-1/2 bg-[#DBEEF6] p-2 mt-4 rounded-[6px] flex flex-col gap-[18px] ">
                          {selectedttibute?.map((op, index) => (
                            ...
                            >
                              ...
                              <span
                                onClick={() => handleDeleteOption(op, index)}
                              >
                                <svg
                                  width="24"
                                  height="24"
...
export default React.forwardRef(AddMultiOptionAttributeModal);
