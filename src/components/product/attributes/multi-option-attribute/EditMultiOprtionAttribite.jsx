...
function EditMultiOprtionAttribite({}, ref) {
  ...
  const getAttOptionQuery = useMutation((id) => singleAttribute(id), {
    onSuccess: (res) => {
      formik.setFieldValue("other_option_ids", res?.data?.data?.options);
      setoptions(res?.data?.data?.options);
      setOptionSearch(res?.data?.data?.options);
    },
  });
  ...
  useImperativeHandle(ref, () => ({
    opeModal(item, index) {
      ...
      if (fromEdit()) {
        if (item?.options) {
          const getOptionIds = item?.options?.map((item) => item?.id);
          setselectedttibute(getOptionIds);
        } else {
          setselectedttibute(item?.option_ids);
        }
      }
      setAttrIndex(item, index);
      if (fromEdit()) {
        getAttOptionQuery.mutate(item?.attrData?.id);
      } else {
        setoptions(item?.other_option_ids);
        setOptionSearch(item?.other_option_ids);
      }

      //   selectedttibute(prev=> [...prev,item?.option_ids])
    },
  }));
  ...
  const formik = useFormik({
    ...
    onSubmit: (data) => {
      data.option_ids = selectedttibute;

      let attributes = productData?.attributes ? productData?.attributes : [];

      // set attr in attrdata for shoe in cart item
      let setAttr = [];

      attributes[index] = data;
      if (data?.option_ids?.length == 0) {
        attributes = attributes.filter((item, indexx) => indexx != index);
      } else {
        options?.map((item) => {
          attributes[index].option_ids?.map((op) => {
            item?.id == op && setAttr.push(item);
          });
        });
        attributes[index].attrData.options = setAttr;
      }

      console.log("savsavaaasfdsfvasv", attributes[index]);
  ...
  useEffect(() => {
    ...
    !fromEdit() &&
      attrData?.option_ids &&
      setselectedttibute(attrData?.option_ids);
  ...
  const handleSelectedAttribute = (item) => {
    setselectedttibute((prev) => [...prev, item?.id]);
  };
  ...
  const handleSearchOption = (s) => {
    console.log("sdcdsdvdsv", attrData?.other_option_ids);
    const re = RegExp(`.*${s.toLowerCase().split("").join(".*")}.*`);

    setOptionSearch(
      options.filter((op) => op?.name_fa.toLowerCase().match(re))
    );
  };

  const handleDeleteOption = (item, index) => {
    console.log("vdsvdsv", options);
    setselectedttibute((prev) => prev.filter((itt) => item?.id != itt));
  };
                  ...
                  <div className="bg-white  text-right shadow-xl rounded-[10px]  w-full flex flex-col">
                      ...
                      <div className="w-full flex flex-row gap-6 justify-between">
                        <div className=" w-1/2 bg-[#DBEEF6] p-2 mt-4 rounded-[6px]">
                          <div className="w-full z-10 flex flex-col justify-start items-start gap-[18px] overflow-scroll hide-scrollbar h-96 p-2">
                            ...
                            {optionSearch?.map(
                              (op) =>
                                !selectedttibute.includes(op?.id) && (
                                  <div
                                    onClick={() => handleSelectedAttribute(op)}
                                    className={`w-full cursor-pointer transition-all  ${"bg-white"}   text-sm rounded-lg p-4 h-[44px] flex justify-start items-center font-bold ${"text-[#003E43]"}`}
                                  >
                                    {op?.name_fa}
                                  </div>
                                )
                        ...
                        <div className=" w-1/2 bg-[#DBEEF6] p-2 mt-4 rounded-[6px] flex flex-col gap-[18px] ">
                          {options?.map((op, index) =>
                            selectedttibute?.map((selectedOp) =>
                              op?.id == selectedOp ? (
                                <div
                                  className={`w-full flex flex-row justify-between cursor-pointer transition-all  ${"bg-white"}   text-sm rounded-lg p-4 h-[44px] flex justify-start items-center font-bold ${"text-[#003E43]"}`}
                                >
                                  <div className="flex flex-row gap-2 items-center">
                                    ...
                                    <span>{op?.name_fa}</span>
                                  </div>

                                  <span
                                    onClick={() =>
                                      handleDeleteOption(op, index)
                                    }
                                  >
                                    <svg
                                      width="24"
                    ...
                    <div className="w-full rounded-md bg-white p-4 flex justify-between items-center">
                      ...
                      <button
                        ...
                        className={`text-[#F9FCFD] w-[115px] ${
                          selectedttibute.length == 0
                            ? "bg-red-700"
                            : "bg-primary "
                        } flex justify-center items-center rounded-md gap-1 h-11`}
                      >
                        <svg
                          width="24"
                          height="24"
                        ...
                        {selectedttibute.length == 0 ? "حذف" : "ثبت ویرایش"}
                      </button>
                    </div>
                  </div>
                </Dialog.Panel>
...
export default React.forwardRef(EditMultiOprtionAttribite);
