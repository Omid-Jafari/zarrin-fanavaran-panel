...
import { blogsFilterData } from "../../../api/ApiClient";
import CustomRadioBtn from "../../common/customRadioBtn";
import Loading from "../../elements/loading";

const BlogProductsModal = (props) => {
  const { blogModal, formik, setBlogModal } = props;
  const [filterData, setFilterData] = useState("");
  const [blogsData, setBlogsData] = useState({ show: false, data: [] });
  const blogsFilterDataMutation = useMutation(blogsFilterData, {
    onSuccess: (res) => {
      setBlogsData((prev) => {
  ...
  useEffect(() => {
    blogsFilterDataMutation.mutate({ filterData });
  }, [filterData]);
  return (
    <Transition.Root show={blogModal} as={Fragment}>
      <Dialog
        as="div"
        className="relative z-50"
        onClose={() => setBlogModal(!blogModal)}
      >
        <Transition.Child
          as={Fragment}
            ...
            >
              <Dialog.Panel className="relative transform overflow-hidden rounded-[8px] w-3/5  bg-white shadow-xl transition-all sm:my-8">
                <div className="bg-cyann w-full p-4 font-medium font-KalamehMed text-white">
                  تنظیمات دستی و خودکار مقالات مرتبط:
                </div>
                <div className="w-full flex flex-col items-start gap-4 p-4">
                  <div className="flex w-full items-center gap-3">
                    <CustomRadioBtn
                      clickFunc={() => {
                        formik.values.blog_relatable === 0
                          ? formik.setFieldValue("blog_relatable", 1)
                          : formik.setFieldValue("blog_relatable", 0);
                      }}
                      checkCondition={formik.values.blog_relatable === 1}
                    />
                    <p className="font-medium font-KalamehMed">
                      نمایش مقالات مرتبط برای این محصول
                    </p>
                  </div>
                  ...
                  <div className="w-full rounded-lg bg-blue-lightt flex flex-col p-4 gap-2 ">
                    {formik.values.blogs?.map((selectedBlogs) => (
                        ...
                        <svg
                          ...
                          onClick={() =>
                            formik.setFieldValue(
                              "blogs",
                              formik.values.blogs?.filter(
                                (item) => item !== selectedBlogs
                              )
                            )
                          }
                        >
                    ...
                    >
                      ...
                      {blogsData?.show && (
                        <div className="absolute z-10 w-full left-0 top-12 flex flex-col max-h-36 bg-white rounded-[6px] border overflow-y-auto">
                          {blogsFilterDataMutation?.isLoading ? (
                            <div className="w-full flex justify-center">
                              <Loading className="w-14 h-14 text-blacklead animate-pulse" />
                            </div>
                          ) : (
                            blogsData?.data?.map((blog) => {
                              const check = formik.values.blogs.indexOf(blog);
                              if (check === -1) {
                                return (
                                  <div
                  ...
                  <div className="flex w-full items-center gap-3">
                    <CustomRadioBtn
                      clickFunc={() => {
                        formik.values.blog_related_type === "SEMI_AUTO"
                          ? formik.setFieldValue("blog_related_type", "MANUAL")
                          : formik.setFieldValue(
                              "blog_related_type",
                              "SEMI_AUTO"
                            );
                    ...
                    />
                    <p className="font-medium font-KalamehMed">
                      سایر مقالات مرتبط را به صورت خودکار اضافه کن
                    </p>
                  </div>
                  <p className="text-sm text-primary">
                    *در صورت خاموش بودن این گزینه، فقط مقالاتی که به صورت دستی
...
export default BlogProductsModal;
