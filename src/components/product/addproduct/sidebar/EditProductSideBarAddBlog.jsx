...
function EditProductSideBarAddBlog({}, ref) {
  const [blogTypee, setblogTypee] = useState();
  ...
  useImperativeHandle(ref, () => ({
    opeModal(item, index, type) {
      console.log("vsdvsvsssscsdv", item);
  ...
  const handleType = (item, type) => {
    console.log("Svsvsvsd", type);
    if (type == blogType.OUTSOURCE) {
      console.log(
        "vsdvsvsscsdv",
        productData?.sidebars.outsource.indexOf(item)
      );
      setindex(productData?.sidebars.outsource.indexOf(item));
      setblogTypee(blogType?.OUTSOURCE);
      OutSourceData(item);
      console.log("SDVsdvsdvsdvsdv");
      if (fromEdit()) {
        // formik.values.poster_id = item?.media?.poster.id;
        formik.setFieldValue("poster_id", item?.media?.poster.id);
        formik.setFieldValue("media", item?.media);
      }
    } else {
      setblogTypee(blogType?.INSOURCE);
      InSourceData(item);
      setindex(productData?.sidebars.insource.indexOf(item));
      console.log(
        "vsdvsvsscsdv_in",
        productData?.sidebars.insource.indexOf(item)
      );
    }
  };

  const OutSourceData = (item) => {
  ...
  const InSourceData = (item) => {
    console.log("sdvsdvsvsdvsdv", item);
    formik.setFieldValue("status", item?.status);
    formik.setFieldValue("id", item?.id);
    if (fromEdit() && item?.blog) {
      setslectedBlog(item?.blog);
    } else {
      setslectedBlog(item?.blogItem);
    }
  ...
  const formik = useFormik({
    ...
    onSubmit: (data) => {
      if (fromEdit()) {
        data.media.poster = data.icon;
      }
      let sidebars = productData.sidebars;
      sidebars[blogTypee.toLowerCase()][index] = data;

      console.log("DSdvsdvvsdvsdv", sidebars[blogTypee.toLowerCase()][index]);
      handleAddData({ sidebars });

      seticon(null);
      setOpen(false);
      formik.resetForm();
      console.log("SDvdsvdsvsdvsdvsdv", data);
    },
    validate: (err) => {},
  });
  console.log("Sdvsdscascsavdsv", productData);
  const formikInSource = useFormik({
    ...
    onSubmit: (data) => {
      ...
      let sidebars = productData.sidebars;
      // data.icon=icon
      data.title = data.blogItem?.name;
      data.itemBlog = sidebars[blogTypee.toLowerCase()][index].blog;
      sidebars[blogTypee.toLowerCase()][index] = data;
      console.log("DSgdfsbfdbdfb", data);
                  ...
                  <Dialog.Panel className="relative w-2/4 transition-all  flex flex-col items-start ">
                    <div className="bg-white  text-right shadow-xl rounded-[10px]  w-full flex flex-col">
                      <div className="w-full h-[60px] bg-[#4FB3BF] rounded-t-[10px] flex justify-start p-3 items-center">
                        <p className="font-Kalameh text-white">ادیت مقاله:</p>
                      </div>
...
export default React.forwardRef(EditProductSideBarAddBlog);
