...
function MediaSettingItem({ settingItem }) {
  const queryClient = useQueryClient();
  const [media, setmedia] = useState(null);
  const [OpenForChangeMedia, setOpenForChangeMedia] = useState(false);
  ...
  const selectedSiteLogo = (item) => {
    setmedia(item?.file);
    formik.values.value = item.id;
    settingItem.media.file = item?.file;
    console.log("Sdvsdvsdvsd", item?.file);
  };

  return (
    <div className="w-3/6 flex flex-row gap-4 items-end">
      <AddMediaCategory
        open={OpenForChangeMedia}
        setOpen={setOpenForChangeMedia}
        selectedItem={(e) => selectedSiteLogo(e)}
      />
      <div className="w-full flex flex-row justify-center items-center gap-[18px]">
        <img className="w-full h-full" src={settingItem?.media?.file} />
        {/* <input
      ...
      <button
        className="flex w-64 items-center justify-center gap-1.5 h-11 bg-primary hover:bg-blacklead transition-colors duration-500 px-6 text-white rounded-[4px] font-KalamehMed font-medium"
        onClick={() => setOpenForChangeMedia(true)}
      >
        <>
...
export default MediaSettingItem;
