...
import {
  getSettingsSelectBoxOptionData,
  sendSettingsData,
} from "../../api/ApiClient";
import Loading from "../elements/loading";

function SelectBoxRemoteSettingItem({ settingItem }) {
  ...
  const getSettingsSelectBoxOptionDataQuery = useMutation(
    (path) => getSettingsSelectBoxOptionData(path),
    {
      onSuccess: (res) => {
        setRemoteOptions(res?.data?.data);
      },
    }
  );

  useEffect(() => {
    getSettingsSelectBoxOptionDataQuery.mutate(
      settingItem?.properties?.remote_url?.slice(13)
    );
    console.log(
      "sdvsdvsdvsdvsd",
      settingItem?.properties?.remote_url?.slice(13)
    );
  }, [settingItem?.properties?.remote_url]);

  return (
    <div className="w-2/6 flex flex-row gap-4 items-center">
...
export default SelectBoxRemoteSettingItem;
