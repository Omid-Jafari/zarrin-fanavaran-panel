...
const NewCoupon = (props, ref) => {
  ...
  const postCouponMutate = useMutation(postCoupon, {
    onSuccess: () => {
      setOpen(false);
      formik.resetForm();
      mutateCoupons();
  ...
  const formik = useFormik({
    initialValues: {
      ...
      all_products: 1,
      notify_users: 0,
      expire_at: new DateObject({ calendar: persian, locale: persian_fa }),
      product_ids: [],
      user_ids: [],
      category_ids: [],
    },
    ...
    onSubmit: (data) => {
      data.expire_at = data?.expire_at
        ?.setSecond(0)
        .format("YYYY/MM/DD HH:mm:ss");

      postCouponMutate.mutate(data);
    },
  });
              ...
              <Dialog.Panel className="relative transform rounded-[8px] w-1/2 bg-white shadow-xl transition-all sm:my-8 flex flex-col items-start">
                ...
                <div className="p-4 flex gap-3 w-full mt-4">
                  <div className="flex flex-col gap-5 font-medium font-KalamehMed">
                    <div className="h-11 flex items-center gap-2">نام تگ:</div>
                    {formik.errors.name && formik.touched.name && (
                      <div className="text-red-600 text-sm h-5"></div>
                    )}
                    <div className="h-11 flex items-center gap-2">مقدار:</div>
                    ...
                    <div className="h-11 flex items-center gap-2">کاربران:</div>
                    {formik.errors.user_ids && formik.touched.user_ids && (
                      <div className="text-red-600 text-sm h-5"></div>
                    )}
                    <div className="h-11 flex items-center gap-2">دسته ها:</div>
                    {formik.errors.category_ids &&
                      formik.touched.category_ids && (
                        <div className="text-red-600 text-sm h-5"></div>
                      )}
                    <div className="h-11 flex items-center gap-2">محصولات</div>
                  ...
                  <div className="flex-1 flex flex-col gap-5 font-medium font-KalamehMed">
                    ...
                    <div className="w-full flex relative" onBlur={handleBlur}>
                      <input
                        type="text"
                      ...
                      >
                        ...
                        <button
                          onClick={() => {
                            formik.setFieldValue("discount_type", "PERCENT");
                            setChooseDiscount(false);
                          }}
                          className="h-11 outline-none w-full text-start"
                        >
                    ...
                    >
                      <DatePicker
                        ...
                        onChange={(e) => formik.setFieldValue("expire_at", e)}
                        calendar={persian}
                        locale={persian_fa}
                ...
                <div
                  className="flex items-center gap-4 cursor-pointer p-5 font-medium font-KalamehMed"
                  onClick={() => {
                    formik.values.notify_users === 1
                      ? formik.setFieldValue("notify_users", 0)
                      : formik.setFieldValue("notify_users", 1);
                  }}
                >
                  <div
                    className={`w-[12px] h-[12px] rounded-[2px] bg-[#C4C7C7] relative cursor-pointer`}
                  >
                    {formik.values.notify_users === 1 && (
                      <div className="w-[11.5px] h-[7px] border-l-[1.5px] border-b-[1.5px] border-black -rotate-45"></div>
                    )}
                  </div>
                  ارسال پیامک برای کاربران انتخاب شده
                </div>
...
export default React.forwardRef(NewCoupon);
