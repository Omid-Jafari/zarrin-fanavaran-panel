...
import {   approveAnswer, approveComment,
    approveQuestion,
    editComment,
    singleAnswer,
    singleComment,
    singleQuestion, } from "../../../api/ApiClient";
import Loading from "../../elements/loading";
import QuestionRejectModal from "../QuestionRejectModal";
import EditQuestionModal from "../EditQuestionModal";
import AnswerRejectModal from "./AnswerRejectModal";
import EditAnswerModal from "./EditAnswerModal";

const SingleAnswersModal = (props, ref) => {
  ...
  const singleQuestionMutate = useMutation((id) => singleAnswer(id), {
    onSuccess: (res) => {
      setAnswerData(res?.data?.data);
    },
  });
  const editCommentMutate = useMutation(
    (data) => editComment(answerData?.id, data),
    {
      onSuccess: (res) => {},
    }
  );
  const approveQuestionMutate = useMutation((id) => approveAnswer(id), {
    onSuccess: () => {
      setOpen(false);
    },
  });
  ...
  const OpenQuestionModal = () => {
    EditQuestionModalRef.current.openModal(answerData);
  };
                    ...
                    <>
                      <div className="w-full flex justify-between items-center ">
                        <div className="px-3 py-2 bg-blue-lightt rounded-md h-10 flex items-center text-sm">
                          {answerData?.status_info?.name}
                        </div>
                      </div>
                      <div className="text-sm ">
                        {answerData?.related?.name}
                      </div>
                      ...
                      {formik.errors.body && formik.touched.body && (
                        <div className="text-red-600 text-sm">
                          {formik.errors.body}
                              ...
                              <p className="font-Kalameh text-[14px] text-[#222427]">
                        پاسخ سوال :
                      </p>
                      <div className="w-full flex flex-col gap-4">
                          ...
                          <>
                            <textarea
                              className="bg-[#EFF1F1] rounded-lg w-full p-4 outline-none"
                              rows={3}
                              placeholder="بدنه"
                              id="body"
                              name="body"
                      ...
                      <div className="w-full bg-blue-lightt rounded p-3 flex items-center gap-3 ">
                        ...
                        <button
                          onClick={() => {
                            formik.handleSubmit();
                            approveQuestionMutate.mutate(answerData?.id);
                          }}
...
export default React.forwardRef(SingleAnswersModal);
