...
import { editQuestion } from "../../api/ApiClient";
import { CopyIem } from "../../utils/Copy";

import Loading from "../elements/loading";

function EditQuestionModal({updateData}, ref) {
  const [open, setOpen] = useState(false);
  const [questionData, setQuestionData] = useState([]);
  const editQuestionMutation = useMutation(
    (data) => editQuestion(questionData?.id, data),
    {
      onSuccess: (res) => {
                  ...
                  <div className=" flex flex-col w-full">
                    <div className="bg-[#DBEEF6] rounded-[6px] p-4">
                      <div className="flex flex-row justify-between items-center ">
                        <p className="text-[#222427] ">متن سئوال:</p>
                        <div className="text-[#222427] flex flex-row gap-2 cursor-pointer"
                        onClick={()=>CopyIem(formik.values?.body)}
                        >
                          <svg
                            width="24"
                            height="24"
...
export default React.forwardRef(EditQuestionModal);
